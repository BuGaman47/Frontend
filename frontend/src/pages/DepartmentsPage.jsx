...
/* 
1. การดึงข้อมูลเริ่มต้น (Initial Fetch): 
   - ใช้ `useEffect` เพื่อเรียก API `departmentService.getAll()` ทันทีที่โหลดหน้าเสร็จ
   - นำข้อมูลแผนกทั้งหมด (พร้อมจำนวนสมาชิก) มาแสดงผลเป็นกลุ่ม Card ด้านบน (Master View)

2. การแสดงผลแบบ Master-Detail:
   - เมื่อผู้ใช้คลิกที่ Card แผนก (`handleSelect`) จะเก็บข้อมูลแผนกที่ถูกเลือกไว้ใน State `selected`
   - จากนั้นจะเรียก API เพื่อดึง "รายชื่อผู้ใช้ทั้งหมด" ที่อยู่ในแผนกนั้นๆ มาแสดงผลเป็นตารางด้านล่าง (Detail View)
   - หากคลิกที่แผนกเดิมที่กำลังเปิดอยู่ จะเป็นการปิดตาราง Detail (Toggle)

3. การจัดการแผนกพิเศษ (Null Department):
   - ระบบมีการจัดการกลุ่ม "ผู้ใช้ที่ยังไม่ได้สังกัดแผนกใดเลย" (id === 'null') ไว้เป็นกรณีพิเศษ
   - โดยจะแยกไปเรียก API `departmentService.getUsersWithoutDept()`

4. การจัดการสถานะ (State Management):
   - มีการแยก State ของ Loading และ Error ทำให้เวลาคลิกดูรายละเอียด หน้าเว็บจะขึ้น Loading แค่ส่วนล่าง ไม่โหลดทับทั้งหน้า
*/
